...
export default {
  ...
  methods: {
    ...
    _selectNode(node) {
      ...
      if (isFullyChecked) {
        ...
        while ((curr = curr.parentNode) !== NO_PARENT_NODE) {
          if (curr.children.filter((_) => !_.isDisabled).every(this.isSelected))
            this.addValue(curr);
          else break;
        }
      }
    ...
    getValue() {
      ...
      if (formatter === "object")
        raws = this.internalValue.map((id) => this.getNode(id));
    ...
    fixSelectParentNode(queue, nextSelectedNodeIds) {
      ...
      while (queue.length) {
        ...
        if (!(parentNode.id in map)) {
          ...
          map[parentNode.id] = parentNode.children.filter(
            (child) => !child.isDisabled,
          ).length; // need exclude disabled chid
        }
    ...
    fixSelectedNodeIds(nodeIdListOfPrevValue) {
      ...
      if (
        ...
        this.fixSelectParentNode(
          nodeIdListOfPrevValue.slice(),
          nextSelectedNodeIds,
        );
      } else if (this.valueConsistsOf === ALL_WITH_INDETERMINATE) {
    ...
    extractCheckedNodeIdsFromValue() {
      ...
      if (formatter === "id") {
        if (combain === "string")
          return this.value.split(this.delimiter).filter(Boolean);
      ...
      if (typeof valued[0] !== "object") {
        console.warn(
          `${this.valueFormat} is suit for data. it turn to string.`,
        );
        return valued;
      }
