import { NO_PARENT_NODE, ALL_CHILDREN, ALL_DESCENDANTS, LEAF_DESCENDANTS, LEAF_CHILDREN } from '../constants';
import { createMap, shadowArrayCopy, match, stringifyOptionPropValue } from '../tools';
	...
	methods: {
		...
		initialize() {
			...
			if (Array.isArray(options)) {
				...
				this.forest.optionsTreeNode = this.buildTree(options, this.defaultParentId);

				// 初始化Index，用于排序 8mm
				this.traverseAllNodesByIndexTrue(this.forest.optionsTreeNode, (node, index) => {
					node.index = (node.isRootNode ? [] : node.parentNode.index).concat(index);
		...
		updateNormalizeOptions() {
			this.forest.normalizedOptions = [];
			this.normalize(this.forest.optionsTreeNode, this.forest.normalizedOptions);
		},
		...
		buildTree(options, parentId = '0', level = 0, parentNode = NO_PARENT_NODE) {
			...
			options.forEach((item) => {
				// construct base item
				const { id, parentId: _parentId, label, disabled } = this.normalizer(item);
				...
				const nestedSearchLabel = isRootNode ? lowerCased.label : node.parentNode.nestedSearchLabel + ' ' + lowerCased.label;
				this.$set(node, 'lowerCased', lowerCased); // search case
				this.$set(node, 'nestedSearchLabel', nestedSearchLabel); // search nest case

				// tree construct
				if (node.parentId === parentId) {
					node.ancestors = !isRootNode ? [parentNode].concat(parentNode.ancestors) : []; // set ancestors
					...
					// 设置是否禁用选子级
					this.$set(node, 'hasDisabledDescendants', this.disabled || (this.disableBranchNodes && node.isBranch));
		...
		toggleNodesAll(status) {
			this.traverseAllNodesByIndexTrue(this.forest.normalizedOptions, (node) => {
		...
		traverseDescendantsBFS(parentNode, callback) {
			...
			while (queue.length) {
				...
				if (currNode.isBranch) currNode.children.forEach((item) => queue.push(item));
				callback(currNode);
				queue.shift();
			}
		},
